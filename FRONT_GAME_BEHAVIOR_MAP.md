...
- Later commissioning proves Jaderna's `start_button_lamp` is a simulation-only legacy assumption because the physical start control is an unlit lever. Task 3 preserves that fake only for Version 7 regression compatibility; it must never appear in production and must be removed through a mandatory coordinated Jaderna Display App/profile/test/Chromium change before production adapter enablement. No Jaderna scene mapping or scene ids are invented in Task 3.
