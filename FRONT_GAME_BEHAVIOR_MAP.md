...
| `screen_6/src/App.jsx` | Phase 3 cooling Display App | Connects to the same configured relay URL; wakes and lights its button on `VALVE_COMPLETE`; consumes one active `ENERGY_SEND` for audio and explicit lamp/axis/scene requests, then emits one delayed reset. | The reset delay defaults to 10 seconds and is baked into each immutable release from `energy_send_reset_delay_ms`; duplicate, sleeping, reconnect, and reload paths do not originate reset. |
...
| Energy send press | `{ "type":"trigger", "name":"BUTTON", "id":"ENERGY_SEND", "data":{"pressed":true} }` | Integration Adapter from Quido input 5; Screen 6 keyboard emulator in development | Screen 6 only while active and idle: play `AUDIO_7`, independently request lamp off, axis `stop`, and scene `sleep`, then emit one canonical `RESET` after the configured delay | Source-authoritative id and shape. Duplicate or non-active events are ignored by Screen 6. |
...
| `BUTTON ENERGY_SEND` | No | No | No | No | No | Audio/control only while active and idle | Delayed `GAME_STATE RESET` after an accepted press | Log only |
...
| `active`, no pending sequence | `BUTTON ENERGY_SEND`, `pressed:true` | Plays `AUDIO_7`; preserves the active end state for the configured delay | Energy lamp off, axis `stop`, scene `sleep`, then one canonical `RESET` after 10 seconds by default |
| `sleep`, pending, or reset-published | `BUTTON ENERGY_SEND`, `pressed:true` | Ignored after a prefixed diagnostic log | None |
| Any state | `GAME_STATE RESET` | Enters `sleep` and stops audio | `energy_send_button_lamp=false` only |

There is no third-phase progress state, 30-second no-send timeout, or success screen. The guarded post-press reset timer is Display App-owned; it is canceled by an external reset or page teardown and is never created by connection or reconnection.
...
| Screen 6 handles the first active pressed `ENERGY_SEND` | Preserve current audio and active end-state display for the configured delay, then emit one canonical `GAME_STATE RESET` | Screen 6 independently requests its lamp off, axis `stop`, and scene `sleep`; duplicate events do nothing. |
...
Screen 6 owns only the configurable post-press delay, default 10 seconds. Neither the adapter nor Control App derives a game reset/transition from a timer, command result, connection state, or observed event.
...
| Energy sent | Play local audio; stop ambient/turbine audio; turn off/reset physical effects; front returns to sleep/initial states | Screen 6 accepts the first press only while active, explicitly requests axis `stop` plus scene `sleep`, and emits one delayed reset | Requests have immediate independent results; reset publication does not wait for or infer their results. |
...
This table is exhaustive. There is no separate `shutdown`, `reset`, `energy_sent`, generic phase, zone, overlay, or delta scene. Screen 6 and OLED 4 have the exact `sleep` permissions above. Screen 6's delayed game reset is an event, not a lighting scene; the adapter never translates `ENERGY_SEND` or `GAME_STATE RESET` into lighting control.
...
| Screen 6 handles the first active pressed `ENERGY_SEND` | `turbine_generator_axis stop` and `tepelni_lighting activate_scene sleep`, each with its own UUID; one `GAME_STATE RESET` follows after the configured delay | Same; request results remain independent of reset publication |
...
- Screen 6 sends exactly one explicit `turbine_generator_axis` `stop` request alongside the first active pressed-`ENERGY_SEND` audio handling, holds the end state for the configured delay, and then publishes one reset.
- Every production `RESET` originator owns its explicit physical actions. Screen 6 stops the axis before its delayed reset; the adapter never derives a command from either `ENERGY_SEND` or `GAME_STATE` and forwards only explicit requests.
...
- Existing local/global reset ownership is preserved: OLED 2 local recovery requests `phase1_ready` and its independently owned start lamp on; OLED 4's inactivity reset origin requests axis `stop` plus `sleep`; Screen 6's accepted energy-send origin performs the same explicit physical shutdown before its configurable delayed reset. Receiving reset makes OLED 2 restore its owned lamps and Screen 6 turn its energy-send lamp off; receivers do not duplicate motor or scene requests, and no integration-owned reset exists.
...
Task 5 implemented the original Version 8 control transitions. The later Screen 6 correction keeps those exact control requests and adds only an active/idle guard plus one configurable delayed `RESET`. OLED 2 owns the selected-fuel Phase 1 scenes and its four backlights; its fuel-qualified completion event gives OLED 4 source-owned context without lower-layer memory. OLED 4 owns the coupled-axis and generation-scene completion pair and its inactivity reset actions. Each request has its own UUID/result and logs independently.
